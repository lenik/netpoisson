...
#
# Environment:
#   NETPOISSON_SRCDIR       project root (default: packaging/../..)
#   NETPOISSON_FORCE_LOCAL  skip can-local check (set on a build host by gh-makerelease)
set -euo pipefail

_LIBDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
NETPOISSON_SRCDIR=${NETPOISSON_SRCDIR:-$(cd "$_LIBDIR/../.." && pwd)}
...
find_build_host_file() {
    ...
    for k in $(_kind_fallbacks "$kind"); do
        for base in "$NETPOISSON_SRCDIR/.config/netpoisson" "$HOME/.config/netpoisson"; do
...
cmd_dump() {
    local file=${1:-}
    if [[ -z $file ]]; then
        file=$(find_build_host_file "${NETPOISSON_PACKAGING:-win32}") || {
    ...
    if ((BH_N > 0)); then
        ...
        echo "last_shell=$(last_shell "${NETPOISSON_PACKAGING:-}")"
...
cmd_run() {
    ...
    if [[ ${NETPOISSON_FORCE_LOCAL:-} == 1 ]]; then
        exec "$@"
    fi
    ...
    local k
    for k in $(_kind_fallbacks "$kind"); do
        echo "    $NETPOISSON_SRCDIR/.config/netpoisson/$k.build-host" >&2
        echo "    $HOME/.config/netpoisson/$k.build-host" >&2
    done
    return 2
}
